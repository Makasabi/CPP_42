...
void Bureaucrat::signForm(AForm & form) const {
	...
	try {
		...
		std::cout << this->_name << " couldn't sign " << form.getName() << " because " << ex.what();
	}
}
...
void Bureaucrat::executeForm(AForm const & form) const {
	...
	try {
		form.execute(*this);
		std::cout << _FOREST_GREEN << this->_name << " executed " << form.getName() << _END << std::endl;
	}
	catch (std::exception& ex) {
		std::cout << this->_name << " could not execute form " << form.getName() << " because: " << ex.what();
	}
}
